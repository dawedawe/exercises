...
-- Q: how can I cast a string to an integer?
-- A: use the read function
-- cast a given given string to an integer
-- read "42" :: Int
strSum :: String -> Int
strSum str =
  sum (map read $ words str :: [Int])
...
lowerAndGreater n list =
  let go :: Int -> Int -> [Int] -> String
      go lesser greater [] = show n ++ " is greater than " ++ show greater ++ " elements and lower than " ++ show lesser ++ " elements"
      go lesser greater (x : xs)
        | x > n = go (succ lesser) greater xs
        | x < n = go lesser (succ greater) xs
        | otherwise = go lesser greater xs
   in go 0 0 list
